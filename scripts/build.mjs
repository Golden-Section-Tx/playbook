...
/* The effort scale. Canonical here; EFFORT.md and every legend in the
 * generated files are rendered from it, so the numbers exist once.
 * `scripts/check_play.py` carries its own copy because it must run standalone
 * against a single file — if you change these values, change them there too. */
const EFFORT_SCALE = [
  ['1',  'One meeting',     '0.25'],
  ['2',  'Half a day',      '0.5'],
  ['3',  'One day',         '1'],
...
const EFFORT_VALUES = new Set([...EFFORT_SCALE.map(([sp]) => `${sp} SP`), '\u2014']);
const effortTable = () => [
  '| SP | Effort | Person-days |',
  '|---|---|---|',
  ...EFFORT_SCALE.map(([sp, label, days]) => `| ${sp} | ${label} | ${days} |`),
];
...
for (const cat of CATEGORIES) {
  ...
  for (const file of readdirSync(dir).filter((f) => f.endsWith('.md') && f !== 'README.md').sort()) {
    ...
    for (const k of ['initialEffort', 'ongoingEffort']) {
      if (fm[k] && !EFFORT_VALUES.has(String(fm[k]).trim())) {
        err(`${rel}: ${k} '${fm[k]}' is not a value on the effort scale — see EFFORT.md`);
      }
    }
...
/* --- EFFORT.md ---------------------------------------------------------- */
{
  ...
  emit(join(ROOT, 'EFFORT.md'), out.join('\n'));
}
